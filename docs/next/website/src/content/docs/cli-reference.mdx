...
Clipboard writes from the controlled terminal are delivered only to its current
controller. Direct attach applies them through the client's clipboard writer;
JSON control emits a separate
`{"type":"terminal.clipboard","encoding":"base64","bytes":"..."}` record
containing the clipboard bytes for the caller to apply. Clipboard effects are
not screen frames and are never sent to observers. Without a direct controller,
clipboard writes retain the foreground Herdr client's existing behavior.
