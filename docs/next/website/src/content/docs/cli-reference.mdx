...
`machine add` discovers running remote sessions, selects the only running session automatically, or opens an Up/Down and Enter picker when several are running. Use `--remote-session <name>` for explicit or non-interactive setup. Interactive setup prefills an editable display label: the SSH target for the default session, or `<ssh-target>/<session>` for a named session. Add `--label <label>` to skip that prompt; non-interactive setup accepts the computed default. `machine list` accepts optional `--json` for scripts. See [Connecting machines](/docs/connecting-machines/) for the full setup and connection guide.
