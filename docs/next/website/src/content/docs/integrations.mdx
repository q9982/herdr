...
Grok integration v2 maps `SessionStart` with `source=load` to a native resume transition. This updates the saved conversation when Grok loads a fork into the same pane. Replacement requires a live detected Grok process and a sequenced report; unknown sources cannot replace an existing session, and child-session reports are ignored. Reinstall the Grok integration after upgrading Herdr to update the installed hook. A rewind that keeps the same session ID needs no identity change.
